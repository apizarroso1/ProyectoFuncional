...
public class AppBiblioteca {
	...
	public static void main(String[] args) {
		...
		System.out.println("(Mensaje informativo: para seleccionar las materias de los libros antes hay que a�adirlas."
	...
	private static void mostrarMenu() {
		...
		System.out.println(" 2.Gesti�n de lectores");
	...
	private static void evaluarOpcLectores(int opcion4, Libreria libreria) {
		...
		case 7:
			validar("�Deseas continuar con la b�squeda del lector?");
	...
	private static void evaluarOpcMaterias(int opcion3, Libreria libreria) {
		...
		case 1:
			validar("�Deseas a�adir materias?");
			Materia m = new Materia();
			m.leerDatos();
			if (libreria.altaMateria(m))
				System.out.println("\n materia a�adida");
			else
				System.out.println("\n error al a�adir materia");
			break;
	...
	public static void darDeAltaUnLibro(Libreria libreria) {
		...
		if (!libreria.existeLibro(a)) {
			...
			} while (Libro.leerString("Quieres a�adir otra?(S/N)").equalsIgnoreCase("S"));
	...
	// Funci�n que anula la baja de un libro

	private static void anularBajaLibro(Libreria libreria) {
		// TODO Auto-generated method stub
		Libro l = new Libro();

		// funcion del objeto librer�a
		boolean exito;
		l.leerIsbn();
	...
	// Funci�n que actualiza los libros
	private static void actualizarLibro(Libreria libreria) {
		...
		if (libreria.existeLibro(l)) {
			...
			} while (System.out.println("\n Quieres a�adir otra?(S/N)").equalsIgnoreCase("S"));
	...
	// Funciones de pr�stamos para el lector
	...
	// Funci�n que da de alta a un Lector
	...
	// Funci�n que da de baja al lector
